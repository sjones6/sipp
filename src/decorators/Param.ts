...
const makeParam = (key) => {
  return (
    ...
    const existingParams: any[] = Reflect.getOwnMetadata(PARAMETER_METADATA, target, propertyKey) || [];
...
const makeModelParam = (key) => (Model) => {
  return (
    ...
    const paramConfigs: object = Reflect.getOwnMetadata(PARAMETER_METADATA, target) || {};
